params ["_veh"];
DEBUG = true;
if (DEBUG) then { diag_log "+++ Fuel Vehicle script runned +++"; };
...
_veh addAction [
    "Remplir Camion",
    {
		...
		if (DEBUG) then { diag_log format ["%1: %2 litres de carburant ajoutés", name _this, _cost * _supply_ammont_per_fuel]; };
    },
...
_veh addAction [
    "Vider Camion",
    {
		...
		if (DEBUG) then { diag_log format ["%1: %2 litres de carburant retirés", name _this, _gain * _supply_ammont_per_fuel]; };
    },
    nil, 1.5, true, true, "", { !isNull ([] call KPLIB_fnc_getNearestFob) }
];