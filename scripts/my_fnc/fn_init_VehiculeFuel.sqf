...
_veh addAction [
    "Remplir Camion",
    {
        ...
        if (DEBUG) then { diag_log format ["[DEBUG][VehiculeFuel] %1: %2 carburant ajoutées", name _this_car, _cost * _supply_ammont_per_fuel]; };
    }
];
...
_veh addAction [
    "Vider Camion",
    {
        ...
        if (DEBUG) then { diag_log format ["[DEBUG][VehiculeFuel] %1: %2 carburant retirées", name _this_car, _gain * _supply_ammont_per_fuel]; };
    }
];