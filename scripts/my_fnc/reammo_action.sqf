...
DEBUG = true;
if (DEBUG) then { systemChat "+++ Réarmement script runned +++" };
...
if (DEBUG) then {
	systemChat format ["-- Ammobox contains %1 ammo", _KPammo_in_box];
	systemChat format ["-- Camion contains %1 ammo", _ACEammo_in_car];
};

_ammo_difference = _ammunition_supply_limit - _ACEammo_in_car;
_cost = _ammo_difference / _supply_ammont_per_ammo;
if (DEBUG) then {
	systemChat format ["-- Ammobox difference is %1", _ammo_difference];
	systemChat format ["-- Ammobox cost is %1", _cost];
};
...
if (DEBUG) then { systemChat format ["-- Réarmement effectué : %1 munitions ajoutées au camion.", _cost * _supply_ammont_per_ammo] };