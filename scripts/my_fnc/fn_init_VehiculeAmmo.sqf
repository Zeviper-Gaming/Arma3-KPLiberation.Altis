...
_veh addAction [
    "Remplir Camion",
    {
        ...
        if (DEBUG) then { diag_log format ["[DEBUG][VehiculeAmmo] %1: %2 munitions ajoutées", name _this_car, _cost * _supply_ammont_per_ammo]; };
    }
];
...
_veh addAction [
    "Vider Camion",
    {
        ...
        if (DEBUG) then { diag_log format ["[DEBUG][VehiculeAmmo] %1: %2 munitions retirées", name _this_car, _gain * _supply_ammont_per_ammo]; };
    }
];