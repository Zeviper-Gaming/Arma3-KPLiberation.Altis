...
DEBUG = false;
if (DEBUG) then { diag_log "+++ Refuel script runned +++" };
...
if (DEBUG) then {
	diag_log format ["-- Ammobox = %1", _caisse];
	diag_log format ["-- Fuelbox contains %1 fuel", _KPfuel_in_box];
	diag_log format ["-- Camion contains %1 / %2 liters", _ACEfuel_in_car, _fuel_supply_limit];
};

_fuel_difference = _fuel_supply_limit - _ACEfuel_in_car;
_cost = _fuel_difference / _supply_ammont_per_fuel;
if (DEBUG) then {
	diag_log format ["-- Fuel difference is %1", _fuel_difference];
	diag_log format ["-- Fuel cost is %1", _cost];
};
...
if (DEBUG) then { diag_log format ["-- Réarmement effectué : %1 munitions ajoutées au camion.", _cost * _supply_ammont_per_fuel] };